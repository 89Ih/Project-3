    ...
    <div className=" d-flex flex-column " style={{ gap: 10 }}>
        ...
        <div className=" w-50 align-self-center align-items-center" >
            <div className=" d-flex flex-row   " >
                ...
                <img onClick={sortName} src={Sort} alt={'sort'} className="css-sort" style={{width:30}} />
            </div>
            <h5  className="d-flex justify-content-center m-1">your plan contains <em className="css-color">{courses.length}</em> courses</h5>
        </div>

        <Row style={{ width: '100%', justifyContent: 'center' }}>
            {courses.filter((course) => course.title.toLowerCase().includes(query.toLowerCase())
            ).filter((course) => course.price <= 25).map((course) => (<Col key={course._id}>
                <Card style={{ width: 400, height:450, margin: 10 }}>
                <Link to={`/course/${course._id}`} >
                    <img src={course.image} width={350} height={150} alt={course.title} className="rounded border" />
                </Link>
                    <p>{course.title}</p>
